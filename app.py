...
if uploaded_file is not None:
    ...
    if clustering_technique == "KMeans":
        ...
        config['model_kw']['n_clusters'] = k_selection if not isinstance(k_selection, tuple) else -1
        ...
        config['model_kw']['min_samples'] = st.slider("Minimum Samples:", 1, 30, 5)
    ...
    if st.session_state['cleaned_df'].shape[1] > 0:
        ...
        if clustering_technique in ["KMeans", "GMM"] and config['model_kw'].get('n_clusters', config['model_kw'].get('n_components')) == 3:
